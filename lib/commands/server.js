...
function readified_js(app_path, id){
  ...
  var manifest = parseManifest(path.join(app_path, 'manifest.json'))
    , settings = parseSettings(path.join(app_path, 'settings.yml')) || parseSettings(path.join(parentDir, 'settings.yml'))
  ...
  if (!settings.title) settings.title = name;
  ...
  default_styles = sass.renderSync({
    data: default_styles
  });
